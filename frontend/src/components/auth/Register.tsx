                ...
                <div className="flex justify-center px-6 my-12">
                    <div className="w-full xl:w-3/4 lg:w-11/12 flex shadow-lg rounded-lg overflow-hidden">
                        ...
                        <div className="w-full lg:w-1/2 bg-white p-5 rounded-lg lg:rounded-r-none">
                            <h3 className="pt-4 text-2xl text-center font-semibold">{t("register.0.header")}</h3>
                            <form
                            ...
                            >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <input type="text" placeholder={t("register.0.input.name")} value={name} onChange={e => setName(e.target.value)} className="border p-2 rounded" required />
                                    <input type="text" placeholder={t("register.0.input.lastname")} value={lastname} onChange={e => setLastname(e.target.value)} className="border p-2 rounded" required />
                                </div>
                                <input type="email" placeholder={t("register.0.input.email")} value={email} onChange={e => setEmail(e.target.value)} className="w-full mt-4 border p-2 rounded" required />
                                ...
                                <input type="password" placeholder={t("register.0.input.confirm_password")} value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className="w-full mt-4 border p-2 rounded" required />

                                <select value={country} onChange={handleCountryChange} className="w-full mt-4 border p-2 rounded" required>
                                    <option value="" disabled>{t("register.0.input.country")}</option>
                                ...
                                <select value={city} onChange={e => setCity(e.target.value)} className="w-full mt-4 border p-2 rounded" disabled={!country} required>
                                    <option value="" disabled>{t("register.0.input.city")}</option>
                                    {cities.map(c => <option key={c} value={c}>{c}</option>)}
                                </select>

                                <input type="text" placeholder={t("register.0.input.address")} value={address} onChange={e => setAddress(e.target.value)} className="w-full mt-4 border p-2 rounded" required />
                                <input type="tel" placeholder={t("register.0.input.phone")} value={phone} onChange={e => setPhone(e.target.value)} className="w-full mt-4 border p-2 rounded" required />
